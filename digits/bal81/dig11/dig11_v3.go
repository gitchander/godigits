...
type Digit11_v3 struct{}

var _ dgdr.DigitDrawer = Digit11_v3{}

func (Digit11_v3) Width(height float64) (width float64) {
	width = height * 8 / 24
	return
}

func (d Digit11_v3) DrawDigit(c *gg.Context, x, y float64, digitHeight float64, digit int) {
	...
	var (
		drawT = func(x, y float64) {
			...
			c.QuadraticTo(x-2*dx, y+2*dy, x, y+4*dy)
		}

		draw1 = func(x, y float64) {
			c.MoveTo(x, y)
			c.QuadraticTo(x+2*dx, y+2*dy, x+dx, y+3*dy)
			c.QuadraticTo(x, y+4*dy, x-2*dx, y+2*dy)
			c.LineTo(x, y+4*dy)
		}
